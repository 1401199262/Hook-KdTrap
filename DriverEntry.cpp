...
u32 LastLine = 0;
u32 ThisLine = 0;
DECLSPEC_NOINLINE u64 BpMe(u64 line)
{
	LastLine = ThisLine;
	ThisLine = line;
...
extern"C" void ExceptionHandler(PEXCEPTION_RECORD ExceptionRecord, PCONTEXT Context)
{
	...
	//	{
	//		BpMe(__LINE__);
	//		NTContinue(Context, Context->Rip + 7);
	//		__fastfail(0);
	//	}
	//}
	//
	...
	//{
	//	//DbgPrintEx(0, 0, "PRIVILEGED_INSTRUCTION exception at %llx\n", Context->Rip);
	//
	//	//go into ur backdoor
	//	if (MmIsAddressValid((pv)Context->Rip) && *(USHORT*)(Context->Rip) == 0x220F) // mov cr
	//	{
	//		//DbgPrintEx(0, 0, "catch mov cr3! \n");
	//		BpMe(__LINE__);
	//
	//		//only for mov cr3, rax
	//		//auto g_trueCr3 = __readcr3();
	//		//__writecr3(g_trueCr3);
	//		NTContinue(Context, Context->Rip + 3);
	//		__fastfail(0);
	//	}
	//}
	//
	...
	//{
	//	if (Context->Rip - (u64)DriverEntry < 0x1000)
	//		NTContinue(Context, Context->Rip + 1);
	//
	//	BpMe(__LINE__);
	//}
